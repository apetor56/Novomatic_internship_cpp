...
void Window::renderSettings(glm::vec2& t1, glm::vec2& t2) {
    ImGui_ImplOpenGL3_NewFrame();
    ImGui_ImplGlfw_NewFrame();
    ImGui::NewFrame();

    ImGui::Begin("Settings");
    ImGui::SetWindowPos("Settings", ImVec2(_width - 250, 0));
    ImGui::SetWindowSize("Settings", ImVec2(250, 180));

    ImGui::Text("First triangle");
    ImGui::SliderFloat("x", &t1.x, -_width, _width);
    ImGui::SliderFloat("y", &t1.y, -_height,_height);

    ImGui::Text("Second triangle");
    ImGui::SliderFloat("x ", &t2.x, -_width, _width);
    ImGui::SliderFloat("y ", &t2.y, -_height, _height);
...
void Window::processInput() {
    ...
    if(state == GLFW_PRESS) {
        // std::cout << '[' << mouse.x << ',' << _height - mouse.y << "]\n";
    }
}
